  ...
  on: function on(t, e, that, callback) {
    ...
    fnc = function fnc(e) {
      ...
      if (callback !== undefined) {
        ...
        f = that;
        ua = navigator.userAgent.toLowerCase(); // filter ui.on(object, event, fn) event listeners
...
(function () {
  ui.classnames.Start = function () {
    var arr, total, created, list, alerts, lastAddedWarning; // create arrays
    ...
    total = ui.find('.' + ui.classnames.nameTotal)[0]; // check all pages with xhr
    ...
    ui.on(document, ui.globals.eventAjaxCallback, function () {
      var i, j, re, reStart, reDuplicate, loaded, str, strStart, strLength, html, title, items, jsClass, jsModule, jsKey, jsStyleList; // load js classnames
          ...
          if (jsKey.match(re) !== null && jsKey.match(ui.classnames.jsIgnore) === null) {
            ...
            if (typeof jsClass === 'string') {
              ...
              if (jsStyleList.length > 1) {
                // check styles for multiple classnames
      ...
      ui.each(loaded, function () {
        ...
        reStart = new RegExp(reStart, 'g'); // check duplicates
      ...
      function filterClassnames(that) {
        title = that.split('-')[1]; // Ex: ui-no-*, ui-sm-*
      ...
      total.textContent = arr.filtered.length + ' / ' + created; // empty variables
