...
(function () {
        ...
        events.each(chart, function () {
            ...
            events.each(parts, function (j) {
                ...
                for (i = 0; i < y.length; i += 1) {
                    ...
                    // create paths
                    if (i === 0) {
                        html += 'M' + ((i * posX) + lineCharts.left) + ' ' + posY + ' '; // start path with M (move to)

                    } else {
                        html += 'L ' + ((i * posX) + lineCharts.left) + ' ' + posY + ' '; // repeat points with L (line to)
                    }
                    ...
                    if (i === 0) {
                        ...
                        html += 'Q 0 0, ' + ((i * posX) + lineCharts.left) + ' ' + posY + ' '; // start quadratic bezier curves with Q (control point, end point)

                    } else {
                        html += 'T ' + ((i * posX) + lineCharts.left) + ' ' + posY + ' '; // repeat last quadratic bezier curves with T (end point)
                    }
                    */
    ...
    events.on(window, 'resize', lineCharts.Start);

    // ajax callback loader: requires Ajax JS
    events.on(document, 'ajaxCallbacks', function () {
        if (ajax.classNames.indexOf('line-charts') > -1) { lineCharts.Start(); }
    });
