<main class="container no-gutter">
    <div class="fixed">
        <div class="row">
            <div class="col-12 padding-30">

                <h3>Button Styles</h3>
                <div class="row padding-30-b block-2nd form-lg ui-ease-2nd-btn">
                    <div class="col-3 col-sm-6">
                        <a class="ui-btn" href="#">Button</a>
                    </div>
                    <div class="col-3 col-sm-6">
                        <a class="ui-btn round" href="#">Button</a>
                    </div>
                    <div class="col-3 col-sm-6">
                        <a class="ui-btn circle" href="#">Button</a>
                    </div>
                    <div class="col-3 col-sm-6">
                        <a class="ui-btn circle ui-border-dual" href="#">Button</a>
                    </div>
                    <div class="col-3 col-sm-6">
                        <a class="ui-btn ui-btn-ghost" href="#">Button</a>
                    </div>
                    <div class="col-3 col-sm-6">
                        <a class="ui-btn ui-btn-ghost ui-border-l ui-border-lg" href="#">Button</a>
                    </div>
                    <div class="col-3 col-sm-6">
                        <a class="ui-btn ui-btn-ghost ui-border-b ui-border-dual" href="#">Button</a>
                    </div>
                    <div class="col-3 col-sm-6">
                        <a class="ui-btn ui-btn-ghost ui-border-b ui-border-lg" href="#">Button</a>
                    </div>
                    <div class="col-3 col-sm-6">
                        <a class="ui-btn ui-btn-ghost-border ui-border round" href="#">Button</a>
                    </div>
                    <div class="col-3 col-sm-6">
                        <a class="ui-btn ui-btn-ghost ui-border-dual circle" href="#">Button</a>
                    </div>
                    <div class="col-3 col-sm-6">
                        <a class="ui-btn ui-btn-ghost ui-border" href="#">Button</a>
                    </div>
                    <div class="col-3 col-sm-6">
                        <a class="ui-btn ui-border round" href="#">Button</a>
                    </div>
                </div>

                <h3>Button Themes</h3>
                <div class="row padding-30-b block-2nd form-lg ui-ease-2nd-btn">
                    <div class="col-3 col-sm-6">
                        <a class="ui-btn ui-theme-red ui-fill-dark-100" href="#">Button</a>
                    </div>
                    <div class="col-3 col-sm-6">
                        <a class="ui-btn round ui-theme-orange ui-fill-dark-100" href="#">Button</a>
                    </div>
                    <div class="col-3 col-sm-6">
                        <a class="ui-btn circle ui-theme-yellow ui-fill-dark-100" href="#">Button</a>
                    </div>
                    <div class="col-3 col-sm-6">
                        <a class="ui-btn circle ui-border-dual ui-theme-base ui-fill-dark-100" href="#">Button</a>
                    </div>
                    <div class="col-3 col-sm-6">
                        <a class="ui-btn ui-btn-ghost ui-theme-green ui-fill-dark-100 ui-color" href="#">Button</a>
                    </div>
                    <div class="col-3 col-sm-6">
                        <a class="ui-btn ui-btn-ghost ui-btn-ghost-border ui-border-l ui-border-lg ui-theme-sub ui-fill-light-100 ui-stroke" href="#">Button</a>
                    </div>
                    <div class="col-3 col-sm-6">
                        <a class="ui-btn ui-btn-ghost ui-border-b ui-border-dual ui-theme-base ui-fill-light-100 ui-stroke" href="#">Button</a>
                    </div>
                    <div class="col-3 col-sm-6">
                        <a class="ui-btn ui-btn-ghost ui-border-b ui-border-lg ui-theme-green ui-fill-dark-100 ui-stroke" href="#">Button</a>
                    </div>
                    <div class="col-3 col-sm-6">
                        <a class="ui-btn ui-btn-ghost-border ui-border round ui-theme-orange ui-fill-dark-100" href="#">Button</a>
                    </div>
                    <div class="col-3 col-sm-6">
                        <a class="ui-btn ui-btn-ghost ui-border-dual circle ui-theme-yellow ui-stroke ui-color" href="#">Button</a>
                    </div>
                    <div class="col-3 col-sm-6">
                        <a class="ui-btn ui-btn-ghost ui-border ui-theme-sub ui-fill-light-100 ui-stroke ui-color" href="#">Button</a>
                    </div>
                    <div class="col-3 col-sm-6">
                        <a class="ui-btn ui-border round ui-theme-sub ui-fill-dark-100" href="#">Button</a>
                    </div>
                </div>

                <h3>Button Sizes</h3>
                <div class="row xs-fluid align-l padding-30-b icons-margin-5-l block-2nd ui-ease-2nd-btn">
                    <div class="col-6">
                        <a class="ui-btn ui-btn-ghost ui-btn-lg circle ui-border" href="#">
                            Large Button <svg class="icon right"><use href="#settings"/></svg>
                        </a>
                    </div>
                    <div class="col-6">
                        <a class="ui-btn ui-btn-ghost ui-btn-lg circle ui-border" href="#">
                            Large Button <svg class="icon right"><use href="#settings"/></svg>
                        </a>
                    </div>
                    <div class="col-6">
                        <a class="ui-btn ui-btn-ghost circle ui-border" href="#">
                            Default Button <svg class="icon right"><use href="#settings"/></svg>
                        </a>
                    </div>
                    <div class="col-6">
                        <a class="ui-btn ui-btn-ghost ui-btn-sm circle ui-border" href="#">
                            Small Button <svg class="icon right"><use href="#settings"/></svg>
                        </a>
                    </div>
                    <div class="col-6">
                        <a class="ui-btn ui-btn-ghost ui-btn-xs circle ui-border" href="#">
                            Extra Small Button <svg class="icon right"><use href="#settings"/></svg>
                        </a>
                    </div>
                    <div class="col-6">
                        <a class="ui-btn ui-btn-ghost ui-btn-xxs circle ui-border" href="#">
                            2x Extra Small Button <svg class="icon right"><use href="#settings"/></svg>
                        </a>
                    </div>
                    <div class="col-6">
                        <a class="ui-btn ui-btn-lg circle" href="#">
                            Large Button <svg class="icon right"><use href="#comment"/></svg>
                        </a>
                    </div>
                    <div class="col-6">
                        <a class="ui-btn ui-btn-lg circle" href="#">
                            Large Button <svg class="icon right"><use href="#comment"/></svg>
                        </a>
                    </div>
                    <div class="col-6">
                        <a class="ui-btn circle" href="#">
                            Default Button <svg class="icon right"><use href="#comment"/></svg>
                        </a>
                    </div>
                    <div class="col-6">
                        <a class="ui-btn ui-btn-sm circle" href="#">
                            Small Button <svg class="icon right"><use href="#comment"/></svg>
                        </a>
                    </div>
                    <div class="col-6">
                        <a class="ui-btn ui-btn-xs circle" href="#">
                            Extra Small Button <svg class="icon right"><use href="#comment"/></svg>
                        </a>
                    </div>
                    <div class="col-6">
                        <a class="ui-btn ui-btn-xxs circle" href="#">
                            2x Extra Small Button <svg class="icon right"><use href="#comment"/></svg>
                        </a>
                    </div>
                </div>

                <h3>Square Buttons</h3>
                <div class="padding-30-b align-c ui-theme-sub ui-ease-1st-btn">
                    <a class="ui-btn ui-btn-square ui-btn-xxs round ui-fill-dark-100" href="#">
                        <svg class="icon"><use href="#trash"/></svg>
                    </a>
                    <a class="ui-btn ui-btn-square ui-btn-xs round ui-fill-dark-100" href="#">
                        <svg class="icon"><use href="#trash"/></svg>
                    </a>
                    <a class="ui-btn ui-btn-square ui-btn-sm round ui-fill-dark-100" href="#">
                        <svg class="icon"><use href="#trash"/></svg>
                    </a>
                    <a class="ui-btn ui-btn-square round ui-fill-dark-100" href="#">
                        <svg class="icon"><use href="#trash"/></svg>
                    </a>
                    <a class="ui-btn ui-btn-square ui-btn-lg round ui-fill-dark-100" href="#">
                        <svg class="icon"><use href="#trash"/></svg>
                    </a>
                </div>

                <h3>Button Notifiers</h3>
                <div class="padding-30-b align-c icons-no-opacity">

                    <div class="row ui-ease-2nd-btn">
                        <div class="col-6">
                            <a href="#" class="ui-btn ui-btn-square ui-btn-xxs round" data-ui-notifier="">
                                <svg class="icon"><use href="#user"/></svg>
                            </a>
                            <a href="#" class="ui-btn ui-btn-square ui-btn-xs round" data-ui-notifier="">
                                <svg class="icon"><use href="#user"/></svg>
                            </a>
                            <a href="#" class="ui-btn ui-btn-square ui-btn-sm round" data-ui-notifier="">
                                <svg class="icon"><use href="#user"/></svg>
                            </a>
                            <a href="#" class="ui-btn ui-btn-square round" data-ui-notifier="">
                                <svg class="icon"><use href="#user"/></svg>
                            </a>
                            <a href="#" class="ui-btn ui-btn-square ui-btn-lg round" data-ui-notifier="">
                                <svg class="icon"><use href="#user"/></svg>
                            </a>
                        </div>
                        <div class="col-6">
                            <a href="#" class="ui-btn ui-btn-square ui-btn-xxs circle" data-ui-notifier="">
                                <svg class="icon"><use href="#user"/></svg>
                            </a>
                            <a href="#" class="ui-btn ui-btn-square ui-btn-xs circle" data-ui-notifier="">
                                <svg class="icon"><use href="#user"/></svg>
                            </a>
                            <a href="#" class="ui-btn ui-btn-square ui-btn-sm circle" data-ui-notifier="">
                                <svg class="icon"><use href="#user"/></svg>
                            </a>
                            <a href="#" class="ui-btn ui-btn-square circle" data-ui-notifier="">
                                <svg class="icon"><use href="#user"/></svg>
                            </a>
                            <a href="#" class="ui-btn ui-btn-square ui-btn-lg circle" data-ui-notifier="">
                                <svg class="icon"><use href="#user"/></svg>
                            </a>
                        </div>
                        <div class="col-6">
                            <a href="#" class="ui-btn ui-btn-square ui-btn-xxs round" data-ui-notifier="1">
                                <svg class="icon"><use href="#setting"/></svg>
                            </a>
                            <a href="#" class="ui-btn ui-btn-square ui-btn-xs round" data-ui-notifier="2">
                                <svg class="icon"><use href="#setting"/></svg>
                            </a>
                            <a href="#" class="ui-btn ui-btn-square ui-btn-sm round" data-ui-notifier="4">
                                <svg class="icon"><use href="#setting"/></svg>
                            </a>
                            <a href="#" class="ui-btn ui-btn-square round" data-ui-notifier="11">
                                <svg class="icon"><use href="#setting"/></svg>
                            </a>
                            <a href="#" class="ui-btn ui-btn-square ui-btn-lg round" data-ui-notifier="20">
                                <svg class="icon"><use href="#user"/></svg>
                            </a>
                            <a href="#" class="ui-btn ui-btn-square ui-btn-lg round" data-ui-notifier="3">
                                <svg class="icon"><use href="#comments"/></svg>
                            </a>
                            <a href="#" class="ui-btn ui-btn-square ui-btn-lg round" data-ui-notifier="315">
                                <svg class="icon"><use href="#bell"/></svg>
                            </a>
                        </div>
                        <div class="col-6">
                            <a href="#" class="ui-btn ui-btn-square ui-btn-xxs circle" data-ui-notifier="1">
                                <svg class="icon"><use href="#setting"/></svg>
                            </a>
                            <a href="#" class="ui-btn ui-btn-square ui-btn-xs circle" data-ui-notifier="2">
                                <svg class="icon"><use href="#setting"/></svg>
                            </a>
                            <a href="#" class="ui-btn ui-btn-square ui-btn-sm circle" data-ui-notifier="4">
                                <svg class="icon"><use href="#setting"/></svg>
                            </a>
                            <a href="#" class="ui-btn ui-btn-square circle" data-ui-notifier="11">
                                <svg class="icon"><use href="#setting"/></svg>
                            </a>
                            <a href="#" class="ui-btn ui-btn-square ui-btn-lg circle" data-ui-notifier="20">
                                <svg class="icon"><use href="#user"/></svg>
                            </a>
                            <a href="#" class="ui-btn ui-btn-square ui-btn-lg circle" data-ui-notifier="3">
                                <svg class="icon"><use href="#comments"/></svg>
                            </a>
                            <a href="#" class="ui-btn ui-btn-square ui-btn-lg circle" data-ui-notifier="315">
                                <svg class="icon"><use href="#bell"/></svg>
                            </a>
                        </div>
                    </div>

                </div>

                <h3>Responsive Buttons</h3>
                <div class="padding-30-b form-lg ui-ease-1st-btn">
                    <a class="ui-btn ui-btn-square round ui-btn-xs-fluid" href="#">
                        <svg class="icon"><use href="#filter"/></svg>
                    </a>
                    <a class="ui-btn round ui-btn-md-fluid" href="#">MD Responsive Button</a>
                    <a class="ui-btn round ui-btn-sm-fluid" href="#">SM Responsive Button</a>
                    <a class="ui-btn round ui-btn-xs-fluid" href="#">XS Responsive Button</a>
                </div>

                <h3>Block Level Buttons</h3>
                <div class="row padding-30-b form-lg ui-ease-2nd-btn">
                    <div class="col-6 align-c">
                        <a class="ui-btn ui-btn-xs-fluid round ui-theme-yellow ui-fill-dark-100" href="#">Responsive Button</a>
                    </div>
                    <div class="col-6">
                        <a class="ui-btn block round ui-theme-orange ui-fill-dark-100" href="#">Block Level Button</a>
                    </div>
                </div>

                <h3>Active &amp; Passive Buttons</h3>
                <div class="row padding-30-b block-2nd ui-ease-2nd-btn">
                    <div class="col-3 col-sm-6">
                        <a class="ui-btn ui-btn-active circle" href="#">Active Link</a>
                    </div>
                    <div class="col-3 col-sm-6">
                        <button class="ui-btn ui-btn-active circle">Active Button</button>
                    </div>
                    <div class="col-3 col-sm-6">
                        <input class="ui-btn ui-btn-active circle" value="Active Input">
                    </div>
                    <div class="col-3 col-sm-6">
                        <input class="ui-btn ui-btn-active circle" value="Active Submit">
                    </div>
                    <div class="col-3 col-sm-6">
                        <a class="ui-btn ui-btn-passive circle" href="#">Passive Link</a>
                    </div>
                    <div class="col-3 col-sm-6">
                        <button class="ui-btn ui-btn-passive circle">Passive Button</button>
                    </div>
                    <div class="col-3 col-sm-6">
                        <input class="ui-btn ui-btn-passive circle" value="Passive Input">
                    </div>
                    <div class="col-3 col-sm-6">
                        <input class="ui-btn ui-btn-passive circle" value="Passive Submit">
                    </div>
                </div>

                <h3>Multi Buttons</h3>
                <div class="row padding-30-b block-2nd ui-theme-green ui-ease-2nd-btn">
                    <div class="col-3 col-sm-6">
                        <a href="#" class="ui-btn ui-btn-lg ui-btn-multi round ui-fill-dark-100">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Maecenas et quam nec augue accumsan cursus.</a>
                    </div>
                    <div class="col-3 col-sm-6">
                        <a href="#" class="ui-btn ui-btn-multi round ui-fill-dark-100">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Maecenas et quam nec augue accumsan cursus.</a>
                    </div>
                    <div class="col-3 col-sm-6">
                        <a href="#" class="ui-btn ui-btn-sm ui-btn-multi round ui-fill-dark-100">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Maecenas et quam nec augue accumsan cursus.</a>
                    </div>
                    <div class="col-3 col-sm-6">
                        <a href="#" class="ui-btn ui-btn-xs ui-btn-multi round ui-fill-dark-100">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Maecenas et quam nec augue accumsan cursus.</a>
                    </div>
                </div>

                <h3>ui-pagination Buttons</h3>
                <div class="padding-30-b">

                    <div class="ui-pagination align-c ui-theme-sub ui-ease-1st-btn">
                        <a class="ui-btn ui-btn-xs prev round" href="#">
                            <svg class="icon"><use href="#angle-dual-left"/></svg>
                        </a>
                        <a class="ui-btn ui-btn-xs prev round" href="#">
                            <svg class="icon"><use href="#angle-left"/></svg>
                        </a>
                        <a class="ui-btn ui-btn-xs round" href="#">1</a>
                        <a class="ui-btn ui-btn-xs ui-btn-active round ui-fill-dark-100" href="#">2</a>
                        <a class="ui-btn ui-btn-xs round" href="#">3</a>
                        <a class="ui-btn ui-btn-xs round" href="#">4</a>
                        <a class="ui-btn ui-btn-xs ui-btn-passive round" href="#">...</a>
                        <a class="ui-btn ui-btn-xs round" href="#">10</a>
                        <a class="ui-btn ui-btn-xs next round" href="#">
                            <svg class="icon"><use href="#angle-right"/></svg>
                        </a>
                        <a class="ui-btn ui-btn-xs next round" href="#">
                            <svg class="icon"><use href="#angle-dual-right"/></svg>
                        </a>
                    </div>

                    <div class="ui-pagination align-c ui-theme-sub ui-ease-1st-btn">
                        <a class="ui-btn ui-btn-sm prev round" href="#">
                            <svg class="icon"><use href="#angle-dual-left"/></svg>
                        </a>
                        <a class="ui-btn ui-btn-sm prev round" href="#">
                            <svg class="icon"><use href="#angle-left"/></svg>
                        </a>
                        <a class="ui-btn ui-btn-sm round" href="#">1</a>
                        <a class="ui-btn ui-btn-sm ui-btn-active round ui-fill-dark-100" href="#">2</a>
                        <a class="ui-btn ui-btn-sm round" href="#">3</a>
                        <a class="ui-btn ui-btn-sm round" href="#">4</a>
                        <a class="ui-btn ui-btn-sm ui-btn-passive round" href="#">...</a>
                        <a class="ui-btn ui-btn-sm round" href="#">10</a>
                        <a class="ui-btn ui-btn-sm next round" href="#">
                            <svg class="icon"><use href="#angle-right"/></svg>
                        </a>
                        <a class="ui-btn ui-btn-sm next round" href="#">
                            <svg class="icon"><use href="#angle-dual-right"/></svg>
                        </a>
                    </div>

                    <div class="ui-pagination align-c ui-theme-sub ui-ease-1st-btn">
                        <a class="ui-btn prev round" href="#">
                            <svg class="icon"><use href="#angle-dual-left"/></svg>
                        </a>
                        <a class="ui-btn prev round" href="#">
                            <svg class="icon"><use href="#angle-left"/></svg>
                        </a>
                        <a class="ui-btn round" href="#">1</a>
                        <a class="ui-btn ui-btn-active round ui-fill-dark-100" href="#">2</a>
                        <a class="ui-btn round" href="#">3</a>
                        <a class="ui-btn round" href="#">4</a>
                        <a class="ui-btn ui-btn-passive round" href="#">...</a>
                        <a class="ui-btn round" href="#">10</a>
                        <a class="ui-btn next round" href="#">
                            <svg class="icon"><use href="#angle-right"/></svg>
                        </a>
                        <a class="ui-btn next round" href="#">
                            <svg class="icon"><use href="#angle-dual-right"/></svg>
                        </a>
                    </div>

                    <div class="ui-pagination form-lg align-c ui-theme-sub ui-ease-1st-btn">
                        <a class="ui-btn prev round" href="#">
                            <svg class="icon"><use href="#angle-dual-left"/></svg>
                        </a>
                        <a class="ui-btn prev round" href="#">
                            <svg class="icon"><use href="#angle-left"/></svg>
                        </a>
                        <a class="ui-btn round" href="#">1</a>
                        <a class="ui-btn ui-btn-active round ui-border-dual ui-stroke ui-color" href="#">2</a>
                        <a class="ui-btn round" href="#">3</a>
                        <a class="ui-btn round" href="#">4</a>
                        <a class="ui-btn ui-btn-passive round" href="#">...</a>
                        <a class="ui-btn round" href="#">10</a>
                        <a class="ui-btn next round" href="#">
                            <svg class="icon"><use href="#angle-right"/></svg>
                        </a>
                        <a class="ui-btn next round" href="#">
                            <svg class="icon"><use href="#angle-dual-right"/></svg>
                        </a>
                    </div>

                </div>

                <h3>Button List</h3>
                <div class="row padding-30-b">
                    <div class="col-6">
                        <div class="ui-btn-list align-l icons-margin-10-r form-lg icons-no-opacity ui-theme-green ui-ease-1st-btn">
                            <button class="ui-btn">
                                <svg class="icon ui-color"><use href="#shield"/></svg>Administration
                            </button>
                            <button class="ui-btn">
                                <svg class="icon ui-color"><use href="#clock"/></svg>Recent History
                            </button>
                            <button class="ui-btn">
                                <svg class="icon ui-color"><use href="#settings"/></svg>Tools
                            </button>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="ui-btn-list align-l icons-margin-10-r form-lg icons-no-opacity ui-theme-red ui-ease-1st-btn">
                            <button class="ui-btn round">
                                <svg class="icon ui-color"><use href="#shield"/></svg>Administration
                            </button>
                            <button class="ui-btn round">
                                <svg class="icon ui-color"><use href="#clock"/></svg>Recent History
                            </button>
                            <button class="ui-btn round">
                                <svg class="icon ui-color"><use href="#settings"/></svg>Tools
                            </button>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="ui-btn-list align-l icons-margin-10-r form-lg icons-no-opacity ui-theme-orange ui-ease-1st-btn">
                            <button class="ui-btn ui-btn-ghost round ui-border ui-stroke">
                                <svg class="icon ui-color"><use href="#shield"/></svg>Administration
                            </button>
                            <button class="ui-btn ui-btn-ghost round ui-border ui-stroke">
                                <svg class="icon ui-color"><use href="#clock"/></svg>Recent History
                            </button>
                            <button class="ui-btn ui-btn-ghost round ui-border ui-stroke">
                                <svg class="icon ui-color"><use href="#settings"/></svg>Tools
                            </button>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="ui-btn-list align-l icons-margin-10-r form-lg icons-no-opacity ui-theme-yellow ui-ease-1st-btn">
                            <button class="ui-btn ui-btn-ghost round ui-border-dual ui-stroke">
                                <svg class="icon ui-color"><use href="#shield"/></svg>Administration
                            </button>
                            <button class="ui-btn ui-btn-ghost round ui-border-dual ui-stroke">
                                <svg class="icon ui-color"><use href="#clock"/></svg>Recent History
                            </button>
                            <button class="ui-btn ui-btn-ghost round ui-border-dual ui-stroke">
                                <svg class="icon ui-color"><use href="#settings"/></svg>Tools
                            </button>
                        </div>
                    </div>
                </div>

                <h3>Button Holder</h3>
                <div class="row padding-30-b">
                    <div class="col-6">
                        <div class="ui-btn-holder form-lg ui-theme-base ui-ease-1st-btn">
                            <button class="ui-btn ui-fill-dark-100">1</button>
                            <button class="ui-btn ui-fill-dark-100">2</button>
                            <button class="ui-btn ui-fill-dark-100">3</button>
                            <button class="ui-btn ui-fill-dark-100">4</button>
                            <button class="ui-btn ui-fill-dark-100">5</button>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="ui-btn-holder form-lg ui-theme-sub ui-ease-1st-btn">
                            <button class="ui-btn round ui-fill-dark-100">1</button>
                            <button class="ui-btn round ui-fill-dark-100">2</button>
                            <button class="ui-btn round ui-fill-dark-100">3</button>
                            <button class="ui-btn round ui-fill-dark-100">4</button>
                            <button class="ui-btn round ui-fill-dark-100">5</button>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="ui-btn-holder ui-btn-sm-fluid form-lg ui-ease-1st-btn">
                            <button class="ui-btn round">One</button>
                            <button class="ui-btn round">Two</button>
                            <button class="ui-btn round">Three</button>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="ui-btn-holder ui-btn-xs-fluid form-lg ui-ease-1st-btn">
                            <button class="ui-btn round">One</button>
                            <button class="ui-btn round">Two</button>
                            <button class="ui-btn round">Three</button>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="ui-btn-holder form-lg ui-ease-1st-btn">
                            <button class="ui-btn round ui-border-r">1</button>
                            <button class="ui-btn round ui-border-r">2</button>
                            <button class="ui-btn round ui-border-r">3</button>
                            <button class="ui-btn round ui-border-r">4</button>
                            <button class="ui-btn round ui-border-r">5</button>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="ui-btn-holder form-lg ui-ease-1st-btn">
                            <button class="ui-btn round ui-border">1</button>
                            <button class="ui-btn round ui-border">2</button>
                            <button class="ui-btn round ui-border">3</button>
                            <button class="ui-btn round ui-border">4</button>
                            <button class="ui-btn round ui-border">5</button>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="ui-btn-holder ui-btn-sm-fluid form-lg ui-ease-1st-btn">
                            <button class="ui-btn round ui-border">One</button>
                            <button class="ui-btn round ui-border">Two</button>
                            <button class="ui-btn round ui-border">Three</button>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="ui-btn-holder ui-btn-xs-fluid form-lg ui-ease-1st-btn">
                            <button class="ui-btn round ui-border-dual">One</button>
                            <button class="ui-btn round ui-border-dual">Two</button>
                            <button class="ui-btn round ui-border-dual">Three</button>
                        </div>
                    </div>
                </div>

            </div>
        </div>
    </div>
</main>
