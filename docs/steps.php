<main class="ui-container ui-no-gutter">
    <div class="ui-fixed">
        <div class="row">
            <div class="col-12 ui-padding-30">

                <h3 class="align-c">Steps Examples</h3>

                <div class="ui-padding-30-b">
                    <ul class="ui-steps-bar">
                        <li class="active"></li>
                        <li></li>
                        <li></li>
                    </ul>
                    <span class="sp-15"></span>
                    <ul class="ui-steps-bar ui-theme-yellow ui-color">
                        <li></li>
                        <li class="active"></li>
                        <li></li>
                    </ul>
                </div>

                <div class="ui-padding-30-v">
                    <ul class="ui-steps-bar">
                        <li class="active">Step Name Here</li>
                        <li>Step Name Here</li>
                        <li>Step Name Here</li>
                    </ul>
                    <span class="sp-15"></span>
                    <ul class="ui-steps-bar ui-theme-yellow ui-color">
                        <li>Step Name Here</li>
                        <li>Step Name Here</li>
                        <li class="active">Step Name Here</li>
                    </ul>
                    <span class="sp-15"></span>
                    <ul class="ui-steps-bar ui-theme-yellow ui-color">
                        <li>Step Name Here</li>
                        <li>Step Name Here</li>
                    </ul>
                </div>

                <div class="ui-padding-30-v">
                    <ul class="ui-steps-bar">
                        <li>Step Name Here</li>
                        <li>Step Name Here</li>
                        <li class="active">Step Name Here</li>
                        <li>Step Name Here</li>
                        <li>Step Name Here</li>
                    </ul>
                </div>

                <div class="ui-padding-30-v">
                    <ul class="ui-steps-bar ui-icons-sm">
                        <li>
                            <svg class="ui-icon ui-margin-3-r"><use href="#check-circle"/></svg>
                            Step Name Here
                        </li>
                        <li class="active">
                            <svg class="ui-icon ui-margin-3-r"><use href="#check-circle"/></svg>
                            Step Name Here
                        </li>
                        <li>
                            <svg class="ui-icon ui-margin-3-r"><use href="#check-circle"/></svg>
                            Step Name Here
                        </li>
                        <li>
                            <svg class="ui-icon ui-margin-3-r"><use href="#check-circle"/></svg>
                            Step Name Here
                        </li>
                    </ul>
                </div>

                <h5 class="font-bold align-c ui-margin-15-b">Steps with Tooltip</h5>
                <div class="ui-padding-30-b">
                    <ul class="ui-steps-bar ui-steps-icon">
                        <li>
                            <svg class="ui-icon" data-ui-tooltip title="Completed"><use href="#check-circle-fill"/></svg>
                            Step Name Here
                        </li>
                        <li class="active">
                            <svg class="ui-icon" data-ui-tooltip title="Completed"><use href="#check-circle-fill"/></svg>
                            Step Name Here
                        </li>
                        <li>
                            <svg class="ui-icon" data-ui-tooltip title="Completed"><use href="#check-circle-fill"/></svg>
                            Step Name Here
                        </li>
                        <li>
                            <svg class="ui-icon" data-ui-tooltip title="Completed"><use href="#check-circle-fill"/></svg>
                            Step Name Here
                        </li>
                    </ul>
                    <span class="sp-15"></span>
                    <ul class="ui-steps-bar ui-theme-orange ui-color">
                        <li>
                            <span data-ui-tooltip title="Completed"></span>
                            Step Name Here
                        </li>
                        <li>
                            <span data-ui-tooltip title="Completed"></span>
                            Step Name Here
                        </li>
                        <li>
                            <span data-ui-tooltip title="Completed"></span>
                            Step Name Here
                        </li>
                        <li class="active">
                            <span data-ui-tooltip title="Completed"></span>
                            Step Name Here
                        </li>
                        <li>
                            <span data-ui-tooltip title="Completed"></span>
                            Step Name Here
                        </li>
                    </ul>
                </div>

                <div class="ui-card ui-padding-30 round shadow-lg">

                    <h5 class="font-bold align-c ui-margin-15-b">Steps in Card</h5>

                    <div class="ui-padding-30-b">
                        <ul class="ui-steps-bar">
                            <li class="active"></li>
                            <li></li>
                            <li></li>
                        </ul>
                        <span class="sp-15"></span>
                        <ul class="ui-steps-bar ui-theme-yellow ui-color">
                            <li></li>
                            <li class="active"></li>
                            <li></li>
                        </ul>
                    </div>

                    <div class="ui-padding-30-v">
                        <ul class="ui-steps-bar">
                            <li class="active">Step Name Here</li>
                            <li>Step Name Here</li>
                            <li>Step Name Here</li>
                        </ul>
                        <span class="sp-15"></span>
                        <ul class="ui-steps-bar ui-theme-yellow ui-color">
                            <li>Step Name Here</li>
                            <li class="active">Step Name Here</li>
                            <li>Step Name Here</li>
                        </ul>
                    </div>

                    <div class="ui-padding-30-v">
                        <ul class="ui-steps-bar">
                            <li>Step Name Here</li>
                            <li>Step Name Here</li>
                            <li class="active">Step Name Here</li>
                            <li>Step Name Here</li>
                            <li>Step Name Here</li>
                        </ul>
                    </div>

                    <div class="ui-padding-30-v">
                        <ul class="ui-steps-bar ui-icons-sm">
                            <li>
                                <svg class="ui-icon ui-margin-3-r"><use href="#check-circle"/></svg>
                                Step Name Here
                            </li>
                            <li class="active">
                                <svg class="ui-icon ui-margin-3-r"><use href="#check-circle"/></svg>
                                Step Name Here
                            </li>
                            <li>
                                <svg class="ui-icon ui-margin-3-r"><use href="#check-circle"/></svg>
                                Step Name Here
                            </li>
                            <li>
                                <svg class="ui-icon ui-margin-3-r"><use href="#check-circle"/></svg>
                                Step Name Here
                            </li>
                        </ul>
                    </div>

                    <h5 class="font-bold align-c ui-margin-15-b">Steps with Tooltip</h5>
                    <div class="ui-padding-30-b">
                        <ul class="ui-steps-bar ui-steps-icon">
                            <li>
                                <svg class="ui-icon" data-ui-tooltip title="Completed"><use href="#check-circle-fill"/></svg>
                                Step Name Here
                            </li>
                            <li class="active">
                                <svg class="ui-icon" data-ui-tooltip title="Completed"><use href="#check-circle-fill"/></svg>
                                Step Name Here
                            </li>
                            <li>
                                <svg class="ui-icon" data-ui-tooltip title="Completed"><use href="#check-circle-fill"/></svg>
                                Step Name Here
                            </li>
                            <li>
                                <svg class="ui-icon" data-ui-tooltip title="Completed"><use href="#check-circle-fill"/></svg>
                                Step Name Here
                            </li>
                        </ul>
                        <span class="sp-15"></span>
                        <ul class="ui-steps-bar ui-theme-orange ui-color">
                            <li>
                                <span data-ui-tooltip title="Completed"></span>
                                Step Name Here
                            </li>
                            <li>
                                <span data-ui-tooltip title="Completed"></span>
                                Step Name Here
                            </li>
                            <li>
                                <span data-ui-tooltip title="Completed"></span>
                                Step Name Here
                            </li>
                            <li class="active">
                                <span data-ui-tooltip title="Completed"></span>
                                Step Name Here
                            </li>
                            <li>
                                <span data-ui-tooltip title="Completed"></span>
                                Step Name Here
                            </li>
                        </ul>
                    </div>

                </div>

            </div>
        </div>
    </div>
</main>