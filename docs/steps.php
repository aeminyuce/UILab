<main class="ui-container ui-no-gutter">
    <div class="ui-fixed">
        <div class="ui-row">
            <div class="ui-col-12 ui-p-30">

                <h3 class="ui-h3 ui-align-c">Steps Examples</h3>

                <div class="ui-p-30-b">
                    <ul class="ui-steps-bar">
                        <li class="ui-active"></li>
                        <li></li>
                        <li></li>
                    </ul>
                    <span class="ui-sp-15"></span>
                    <ul class="ui-steps-bar ui-theme-yellow ui-text">
                        <li></li>
                        <li class="ui-active"></li>
                        <li></li>
                    </ul>
                </div>

                <div class="ui-p-30-v">
                    <ul class="ui-steps-bar">
                        <li class="ui-active">Step Name Here</li>
                        <li>Step Name Here</li>
                        <li>Step Name Here</li>
                    </ul>
                    <span class="ui-sp-15"></span>
                    <ul class="ui-steps-bar ui-theme-yellow ui-text">
                        <li>Step Name Here</li>
                        <li>Step Name Here</li>
                        <li class="ui-active">Step Name Here</li>
                    </ul>
                    <span class="ui-sp-15"></span>
                    <ul class="ui-steps-bar ui-theme-yellow ui-text">
                        <li>Step Name Here</li>
                        <li>Step Name Here</li>
                    </ul>
                </div>

                <div class="ui-p-30-v">
                    <ul class="ui-steps-bar">
                        <li>Step Name Here</li>
                        <li>Step Name Here</li>
                        <li class="ui-active">Step Name Here</li>
                        <li>Step Name Here</li>
                        <li>Step Name Here</li>
                    </ul>
                </div>

                <div class="ui-p-30-v">
                    <ul class="ui-steps-bar ui-icons-sm">
                        <li>
                            <svg class="ui-icon ui-m-3-r"><use href="../dist/icons.svg#check-circle"/></svg>
                            Step Name Here
                        </li>
                        <li class="ui-active">
                            <svg class="ui-icon ui-m-3-r"><use href="../dist/icons.svg#check-circle"/></svg>
                            Step Name Here
                        </li>
                        <li>
                            <svg class="ui-icon ui-m-3-r"><use href="../dist/icons.svg#check-circle"/></svg>
                            Step Name Here
                        </li>
                        <li>
                            <svg class="ui-icon ui-m-3-r"><use href="../dist/icons.svg#check-circle"/></svg>
                            Step Name Here
                        </li>
                    </ul>
                </div>

                <h5 class="ui-h5 ui-font-bold ui-align-c ui-m-15-b">Steps with Tooltip</h5>
                <div class="ui-p-30-b">
                    <ul class="ui-steps-bar ui-steps-icon">
                        <li>
                            <svg class="ui-icon" data-ui-tooltip title="Completed"><use href="../dist/icons.svg#check-circle"/></svg>
                            Step Name Here
                        </li>
                        <li class="ui-active">
                            <svg class="ui-icon" data-ui-tooltip title="Completed"><use href="../dist/icons.svg#check-circle"/></svg>
                            Step Name Here
                        </li>
                        <li>
                            <svg class="ui-icon" data-ui-tooltip title="Completed"><use href="../dist/icons.svg#check-circle"/></svg>
                            Step Name Here
                        </li>
                        <li>
                            <svg class="ui-icon" data-ui-tooltip title="Completed"><use href="../dist/icons.svg#check-circle"/></svg>
                            Step Name Here
                        </li>
                    </ul>
                    <span class="ui-sp-15"></span>
                    <ul class="ui-steps-bar ui-theme-orange ui-text">
                        <li>
                            <span data-ui-tooltip title="Completed"></span>
                            Step Name Here
                        </li>
                        <li>
                            <span data-ui-tooltip title="Completed"></span>
                            Step Name Here
                        </li>
                        <li>
                            <span data-ui-tooltip title="Completed"></span>
                            Step Name Here
                        </li>
                        <li class="ui-active">
                            <span data-ui-tooltip title="Completed"></span>
                            Step Name Here
                        </li>
                        <li>
                            <span data-ui-tooltip title="Completed"></span>
                            Step Name Here
                        </li>
                    </ul>
                </div>

                <div class="ui-card ui-p-30 ui-round ui-shadow-lg">

                    <h5 class="ui-h5 ui-font-bold ui-align-c ui-m-15-b">Steps in Card</h5>

                    <div class="ui-p-30-b">
                        <ul class="ui-steps-bar">
                            <li class="ui-active"></li>
                            <li></li>
                            <li></li>
                        </ul>
                        <span class="ui-sp-15"></span>
                        <ul class="ui-steps-bar ui-theme-yellow ui-text">
                            <li></li>
                            <li class="ui-active"></li>
                            <li></li>
                        </ul>
                    </div>

                    <div class="ui-p-30-v">
                        <ul class="ui-steps-bar">
                            <li class="ui-active">Step Name Here</li>
                            <li>Step Name Here</li>
                            <li>Step Name Here</li>
                        </ul>
                        <span class="ui-sp-15"></span>
                        <ul class="ui-steps-bar ui-theme-yellow ui-text">
                            <li>Step Name Here</li>
                            <li class="ui-active">Step Name Here</li>
                            <li>Step Name Here</li>
                        </ul>
                    </div>

                    <div class="ui-p-30-v">
                        <ul class="ui-steps-bar">
                            <li>Step Name Here</li>
                            <li>Step Name Here</li>
                            <li class="ui-active">Step Name Here</li>
                            <li>Step Name Here</li>
                            <li>Step Name Here</li>
                        </ul>
                    </div>

                    <div class="ui-p-30-v">
                        <ul class="ui-steps-bar ui-icons-sm">
                            <li>
                                <svg class="ui-icon ui-m-3-r"><use href="../dist/icons.svg#check-circle"/></svg>
                                Step Name Here
                            </li>
                            <li class="ui-active">
                                <svg class="ui-icon ui-m-3-r"><use href="../dist/icons.svg#check-circle"/></svg>
                                Step Name Here
                            </li>
                            <li>
                                <svg class="ui-icon ui-m-3-r"><use href="../dist/icons.svg#check-circle"/></svg>
                                Step Name Here
                            </li>
                            <li>
                                <svg class="ui-icon ui-m-3-r"><use href="../dist/icons.svg#check-circle"/></svg>
                                Step Name Here
                            </li>
                        </ul>
                    </div>

                    <h5 class="ui-h5 ui-font-bold ui-align-c ui-m-15-b">Steps with Tooltip</h5>
                    <div class="ui-p-30-b">
                        <ul class="ui-steps-bar ui-steps-icon">
                            <li>
                                <svg class="ui-icon" data-ui-tooltip title="Completed"><use href="../dist/icons.svg#check-circle"/></svg>
                                Step Name Here
                            </li>
                            <li class="ui-active">
                                <svg class="ui-icon" data-ui-tooltip title="Completed"><use href="../dist/icons.svg#check-circle"/></svg>
                                Step Name Here
                            </li>
                            <li>
                                <svg class="ui-icon" data-ui-tooltip title="Completed"><use href="../dist/icons.svg#check-circle"/></svg>
                                Step Name Here
                            </li>
                            <li>
                                <svg class="ui-icon" data-ui-tooltip title="Completed"><use href="../dist/icons.svg#check-circle"/></svg>
                                Step Name Here
                            </li>
                        </ul>
                        <span class="ui-sp-15"></span>
                        <ul class="ui-steps-bar ui-theme-orange ui-text">
                            <li>
                                <span data-ui-tooltip title="Completed"></span>
                                Step Name Here
                            </li>
                            <li>
                                <span data-ui-tooltip title="Completed"></span>
                                Step Name Here
                            </li>
                            <li>
                                <span data-ui-tooltip title="Completed"></span>
                                Step Name Here
                            </li>
                            <li class="ui-active">
                                <span data-ui-tooltip title="Completed"></span>
                                Step Name Here
                            </li>
                            <li>
                                <span data-ui-tooltip title="Completed"></span>
                                Step Name Here
                            </li>
                        </ul>
                    </div>

                </div>

            </div>
        </div>
    </div>
</main>