<main class="ui-container ui-no-gutter">
    <div class="ui-fixed">
        <div class="ui-row">
            <div class="ui-col-12 ui-align-c ui-p-30">

                <h3 class="ui-h3">Avatars</h3>
                <div class="ui-p-30-b">

                    <img class="ui-avatar-xs ui-circle" src="../public/img/profile-image.jpg" alt="">
                    <img class="ui-avatar-xs ui-circle" src="../public/img/profile-image2.jpg" alt="">
                    <span class="ui-avatar-xs ui-circle">
                        <svg class="ui-icon"><use href="../dist/icons.svg#user"/></svg>
                    </span>

                    <span class="ui-sp-5 ui-visible-sm"></span>

                    <span class="ui-avatar-xs ui-circle ui-theme-orange ui-fill-dark-100">
                        <svg class="ui-icon"><use href="../dist/icons.svg#user"/></svg>
                    </span>
                    <span class="ui-avatar-xs ui-circle ui-theme-yellow ui-fill-dark-100">
                        <span>HY</span>
                    </span>
                    <span class="ui-avatar-xs ui-circle ui-theme-sub ui-fill-dark-100">
                        <span>EKY</span>
                    </span>

                    <span class="ui-sp-30"></span>

                    <img class="ui-avatar-sm ui-circle" src="../public/img/profile-image.jpg" alt="">
                    <img class="ui-avatar-sm ui-circle" src="../public/img/profile-image2.jpg" alt="">
                    <span class="ui-avatar-sm ui-circle">
                        <svg class="ui-icon"><use href="../dist/icons.svg#user"/></svg>
                    </span>

                    <span class="ui-sp-5 ui-visible-sm"></span>

                    <span class="ui-avatar-sm ui-circle ui-theme-orange ui-fill-dark-100">
                        <svg class="ui-icon"><use href="../dist/icons.svg#user"/></svg>
                    </span>
                    <span class="ui-avatar-sm ui-circle ui-theme-yellow ui-fill-dark-100">
                        <span>HY</span>
                    </span>
                    <span class="ui-avatar-sm ui-circle ui-theme-sub ui-fill-dark-100">
                        <span>EKY</span>
                    </span>

                    <span class="ui-sp-30"></span>

                    <img class="ui-avatar ui-circle" src="../public/img/profile-image.jpg" alt="">
                    <img class="ui-avatar ui-circle" src="../public/img/profile-image2.jpg" alt="">
                    <span class="ui-avatar ui-circle">
                        <svg class="ui-icon"><use href="../dist/icons.svg#user"/></svg>
                    </span>

                    <span class="ui-sp-5 ui-visible-sm"></span>

                    <span class="ui-avatar ui-circle ui-theme-orange ui-fill-dark-100">
                        <svg class="ui-icon"><use href="../dist/icons.svg#user"/></svg>
                    </span>
                    <span class="ui-avatar ui-circle ui-theme-yellow ui-fill-dark-100">
                        <span>HY</span>
                    </span>
                    <span class="ui-avatar ui-circle ui-theme-sub ui-fill-dark-100">
                        <span>EKY</span>
                    </span>

                    <span class="ui-sp-30"></span>

                    <img class="ui-avatar-lg ui-circle" src="../public/img/profile-image.jpg" alt="">
                    <img class="ui-avatar-lg ui-circle" src="../public/img/profile-image2.jpg" alt="">
                    <span class="ui-avatar-lg ui-circle">
                        <svg class="ui-icon"><use href="../dist/icons.svg#user"/></svg>
                    </span>

                    <span class="ui-sp-5 ui-visible-sm"></span>

                    <span class="ui-avatar-lg ui-circle ui-theme-orange ui-fill-dark-100">
                        <svg class="ui-icon"><use href="../dist/icons.svg#user"/></svg>
                    </span>
                    <span class="ui-avatar-lg ui-circle ui-theme-yellow ui-fill-dark-100">
                        <span>HY</span>
                    </span>
                    <span class="ui-avatar-lg ui-circle ui-theme-sub ui-fill-dark-100">
                        <span>EKY</span>
                    </span>

                </div>

                <span class="ui-sp-30"></span>

                <h3 class="ui-h3">Avatar Holder</h3>
                <div class="ui-p-30-b">

                    <div class="ui-avatar-holder ui-hover-scale-more-1st ui-ease-1st-layout">
                        <img class="ui-avatar-sm ui-border-dual ui-circle" src="../public/img/profile-image.jpg" alt="">
                        <img class="ui-avatar-sm ui-border-dual ui-circle" src="../public/img/profile-image2.jpg" alt="">
                        <img class="ui-avatar-sm ui-border-dual ui-circle" src="../public/img/profile-image3.jpg" alt="">
                    </div>

                    <span class="ui-sp-30"></span>

                    <div class="ui-avatar-holder ui-hover-scale-more-2nd ui-ease-2nd-layout">
                        <a href="#"><img class="ui-avatar ui-border-dual ui-circle" src="../public/img/profile-image.jpg" alt=""></a>
                        <a href="#"><img class="ui-avatar ui-border-dual ui-circle" src="../public/img/profile-image2.jpg" alt=""></a>
                        <a href="#"><img class="ui-avatar ui-border-dual ui-circle" src="../public/img/profile-image3.jpg" alt=""></a>
                    </div>

                    <span class="ui-sp-30"></span>

                    <div class="ui-avatar-holder ui-hover-scale-more-2nd ui-ease-2nd-layout">
                        <a href="#"><img class="ui-avatar-lg ui-border-dual ui-circle" src="../public/img/profile-image.jpg" alt=""></a>
                        <a href="#"><img class="ui-avatar-lg ui-border-dual ui-circle" src="../public/img/profile-image2.jpg" alt=""></a>
                        <a href="#"><img class="ui-avatar-lg ui-border-dual ui-circle" src="../public/img/profile-image3.jpg" alt=""></a>
                    </div>

                    <span class="ui-sp-30"></span>

                    <div class="ui-card ui-p-30 ui-round ui-shadow-lg">
                        <h5 class="ui-h5 ui-font-bold">Avatar Holders in Card</h5>

                        <div class="ui-avatar-holder ui-hover-scale-more-1st ui-ease-1st-layout">
                            <img class="ui-avatar-sm ui-border-dual ui-circle" src="../public/img/profile-image.jpg" alt="">
                            <img class="ui-avatar-sm ui-border-dual ui-circle" src="../public/img/profile-image2.jpg" alt="">
                            <img class="ui-avatar-sm ui-border-dual ui-circle" src="../public/img/profile-image3.jpg" alt="">
                        </div>

                        <span class="ui-sp-30"></span>

                        <div class="ui-avatar-holder ui-hover-scale-more-2nd ui-ease-2nd-layout">
                            <a href="#"><img class="ui-avatar-lg ui-border-dual ui-circle" src="../public/img/profile-image.jpg" alt=""></a>
                            <a href="#"><img class="ui-avatar-lg ui-border-dual ui-circle" src="../public/img/profile-image2.jpg" alt=""></a>
                            <a href="#"><img class="ui-avatar-lg ui-border-dual ui-circle" src="../public/img/profile-image3.jpg" alt=""></a>
                        </div>
                    </div>

                    <span class="ui-sp-30"></span>

                    <div class="ui-avatar-holder ui-circle ui-border-dual ui-hover-r-more-2nd ui-ease-2nd-layout">
                        <a href="#"><img class="ui-avatar ui-border-dual ui-circle" src="../public/img/profile-image.jpg" alt=""></a>
                        <a href="#"><img class="ui-avatar ui-border-dual ui-circle" src="../public/img/profile-image2.jpg" alt=""></a>
                        <a href="#"><img class="ui-avatar ui-border-dual ui-circle" src="../public/img/profile-image3.jpg" alt=""></a>
                        <a class="ui-btn ui-btn-square ui-btn-ghost ui-m-5-r ui-circle ui-ease-btn" href="#">+3</a>
                    </div>

                    <span class="ui-sp-5 ui-visible-sm"></span>

                    <div class="ui-avatar-holder ui-circle ui-hover-scale-more-2nd ui-ease-2nd-layout ui-theme-sub ui-fill-dark-100">
                        <a href="#"><img class="ui-avatar ui-border-dual ui-stroke ui-circle" src="../public/img/profile-image.jpg" alt=""></a>
                        <a href="#"><img class="ui-avatar ui-border-dual ui-stroke ui-circle" src="../public/img/profile-image2.jpg" alt=""></a>
                        <a href="#"><img class="ui-avatar ui-border-dual ui-stroke ui-circle" src="../public/img/profile-image3.jpg" alt=""></a>
                        <a href="#"><img class="ui-avatar ui-border-dual ui-stroke ui-circle" src="../public/img/profile-image.jpg" alt=""></a>
                        <a class="ui-btn ui-btn-square ui-btn-ghost ui-m-5-r ui-circle ui-fill-dark-300 ui-ease-btn" href="#">+8</a>
                    </div>

                    <span class="ui-sp-30"></span>

                    <div class="ui-avatar-holder ui-p-5 ui-circle ui-border-dual ui-hover-scale-more-2nd ui-ease-2nd-layout">
                        <a href="#"><img class="ui-avatar ui-border-dual ui-circle" src="../public/img/profile-image.jpg" alt=""></a>
                        <a href="#"><img class="ui-avatar ui-border-dual ui-circle" src="../public/img/profile-image2.jpg" alt=""></a>
                        <a href="#"><img class="ui-avatar ui-border-dual ui-circle" src="../public/img/profile-image3.jpg" alt=""></a>
                        <a class="ui-btn ui-btn-square ui-btn-ghost ui-btn-lg ui-m-2-r ui-circle ui-ease-btn" href="#">+12</a>
                    </div>

                    <span class="ui-sp-5 ui-visible-sm"></span>

                    <div class="ui-avatar-holder ui-p-5 ui-circle ui-hover-scale-more-2nd ui-ease-2nd-layout ui-theme-sub ui-fill-dark-100">
                        <a href="#"><img class="ui-avatar ui-border-dual ui-stroke ui-circle" src="../public/img/profile-image.jpg" alt=""></a>
                        <a href="#"><img class="ui-avatar ui-border-dual ui-stroke ui-circle" src="../public/img/profile-image2.jpg" alt=""></a>
                        <a href="#"><img class="ui-avatar ui-border-dual ui-stroke ui-circle" src="../public/img/profile-image3.jpg" alt=""></a>
                        <a href="#"><img class="ui-avatar ui-border-dual ui-stroke ui-circle" src="../public/img/profile-image.jpg" alt=""></a>
                        <a class="ui-btn ui-btn-square ui-btn-ghost ui-btn-lg ui-m-2-r ui-circle ui-fill-dark-300 ui-ease-btn" href="#">+26</a>
                    </div>

                </div>

            </div>
        </div>
    </div>
</main>