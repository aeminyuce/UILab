<main class="ui-container ui-no-gutter">
    <div class="ui-fixed">
        <div class="ui-row">
            <div class="ui-col-12 ui-p-30-v">

                <h3 class="ui-h3">Carousel Examples</h3>
                <div class="ui-p-30-b">

                    <h5 class="ui-h5 ui-font-bold">HTML Content Carousel</h5>
                    <div class="ui-p-10 ui-round ui-shadow-in-sm ui-theme-gray ui-fill-light-100">

                        <div class="ui-carousel" data-ui-col="1" data-ui-slide="5000">
                            <div class="ui-carousel-slider ui-form-lg ui-ease-2nd-layout ui-ease-layout ui-ease-slow-2x ui-ease-in-out">

                                <div class="ui-carousel-content ui-p-10-h" data-ui-animate="300">

                                        <div class="ui-row ui-sm-fluid">
                                            <div class="ui-col-4 ui-col-md-6">

                                                <div class="ui-row ui-row-gap-xs ui-no-fluid ui-ease-2nd-layout ui-ease-slow ui-ease-in-out">
                                                    <div class="ui-col-6">
                                                        <img class="ui-carousel-animate ui-bring-back ui-img-fluid" src="../public/img/image_09.jpg" alt="">
                                                    </div>
                                                    <div class="ui-col-6">
                                                        <img class="ui-carousel-animate ui-bring-front ui-img-fluid" src="../public/img/image_05.jpg" alt="">
                                                    </div>
                                                    <div class="ui-col-4">
                                                        <img class="ui-carousel-animate ui-bring-l ui-img-fluid" src="../public/img/image_02.jpg" alt="">
                                                    </div>
                                                    <div class="ui-col-4">
                                                        <img class="ui-carousel-animate ui-bring-b ui-img-fluid" src="../public/img/image_03.jpg" alt="">
                                                    </div>
                                                    <div class="ui-col-4">
                                                        <img class="ui-carousel-animate ui-bring-r ui-img-fluid" src="../public/img/image_01.jpg" alt="">
                                                    </div>
                                                </div>

                                            </div>
                                            <div class="ui-col-8 ui-col-md-6">

                                                <div class="ui-carousel-animate ui-bring-r ui-ease-layout ui-ease-slow">
                                                    <h3 class="ui-h3">1- Carousel Html Title</h3>
                                                    <p class="ui-font-16 ui-font-readable">
                                                        Maecenas blandit est ultrices eleifend commodo. Integer eget eros a orci ornare tincidunt et at dolor. Suspendisse nec augue vel mauris malesuada aliquet. Quisque id tincidunt libero.
                                                        <br><br>
                                                        Aenean vel justo sit amet velit dapibus ultrices nec at purus. In non finibus nisi. Vestibulum porta lectus dolor, a ullamcorper lorem feugiat varius.
                                                        <br><br>

                                                        <a href="#" class="ui-btn ui-btn-xs-fluid ui-theme-sub ui-fill-dark-100 ui-circle ui-ease-btn">
                                                            Continue
                                                            <svg class="ui-icon ui-m-5-l"><use href="../dist/icons.svg#arrow-right"/></svg>
                                                        </a>
                                                    </p>
                                                </div>

                                            </div>
                                        </div>

                                </div>
                                <div class="ui-carousel-content ui-p-10-h" data-ui-animate="300">

                                        <div class="ui-row ui-sm-fluid">
                                            <div class="ui-col-4 ui-col-md-6">

                                                <div class="ui-row ui-row-gap-xs ui-no-fluid ui-ease-2nd-layout ui-ease-slow ui-ease-in-out">
                                                    <div class="ui-col-6">
                                                        <img class="ui-carousel-animate ui-bring-back ui-img-fluid" src="../public/img/image_09.jpg" alt="">
                                                    </div>
                                                    <div class="ui-col-6">
                                                        <img class="ui-carousel-animate ui-bring-front ui-img-fluid" src="../public/img/image_05.jpg" alt="">
                                                    </div>
                                                    <div class="ui-col-4">
                                                        <img class="ui-carousel-animate ui-bring-l ui-img-fluid" src="../public/img/image_02.jpg" alt="">
                                                    </div>
                                                    <div class="ui-col-4">
                                                        <img class="ui-carousel-animate ui-bring-b ui-img-fluid" src="../public/img/image_03.jpg" alt="">
                                                    </div>
                                                    <div class="ui-col-4">
                                                        <img class="ui-carousel-animate ui-bring-r ui-img-fluid" src="../public/img/image_01.jpg" alt="">
                                                    </div>
                                                </div>

                                            </div>
                                            <div class="ui-col-8 ui-col-md-6">

                                                <div class="ui-carousel-animate ui-bring-r ui-ease-layout ui-ease-slow">
                                                    <h3 class="ui-h3">2- Carousel Html Title</h3>
                                                    <p class="ui-font-16 ui-font-readable">
                                                        Maecenas blandit est ultrices eleifend commodo. Integer eget eros a orci ornare tincidunt et at dolor. Suspendisse nec augue vel mauris malesuada aliquet. Quisque id tincidunt libero.
                                                        <br><br>
                                                        Aenean vel justo sit amet velit dapibus ultrices nec at purus. In non finibus nisi. Vestibulum porta lectus dolor, a ullamcorper lorem feugiat varius.
                                                        <br><br>

                                                        <a href="#" class="ui-btn ui-btn-xs-fluid ui-theme-sub ui-fill-dark-100 ui-circle ui-ease-btn">
                                                            Continue
                                                            <svg class="ui-icon ui-m-5-l"><use href="../dist/icons.svg#arrow-right"/></svg>
                                                        </a>
                                                    </p>
                                                </div>

                                            </div>
                                        </div>

                                </div>
                                <div class="ui-carousel-content ui-p-10-h" data-ui-animate="300">

                                        <div class="ui-row ui-sm-fluid">
                                            <div class="ui-col-4 ui-col-md-6">

                                                <div class="ui-row ui-row-gap-xs ui-no-fluid ui-ease-2nd-layout ui-ease-slow ui-ease-in-out">
                                                    <div class="ui-col-6">
                                                        <img class="ui-carousel-animate ui-bring-back ui-img-fluid" src="../public/img/image_09.jpg" alt="">
                                                    </div>
                                                    <div class="ui-col-6">
                                                        <img class="ui-carousel-animate ui-bring-front ui-img-fluid" src="../public/img/image_05.jpg" alt="">
                                                    </div>
                                                    <div class="ui-col-4">
                                                        <img class="ui-carousel-animate ui-bring-l ui-img-fluid" src="../public/img/image_02.jpg" alt="">
                                                    </div>
                                                    <div class="ui-col-4">
                                                        <img class="ui-carousel-animate ui-bring-b ui-img-fluid" src="../public/img/image_03.jpg" alt="">
                                                    </div>
                                                    <div class="ui-col-4">
                                                        <img class="ui-carousel-animate ui-bring-r ui-img-fluid" src="../public/img/image_01.jpg" alt="">
                                                    </div>
                                                </div>

                                            </div>
                                            <div class="ui-col-8 ui-col-md-6">

                                                <div class="ui-carousel-animate ui-bring-r ui-ease-layout ui-ease-slow">
                                                    <h3 class="ui-h3">3- Carousel Html Title</h3>
                                                    <p class="ui-font-16 ui-font-readable">
                                                        Maecenas blandit est ultrices eleifend commodo. Integer eget eros a orci ornare tincidunt et at dolor. Suspendisse nec augue vel mauris malesuada aliquet. Quisque id tincidunt libero.
                                                        <br><br>
                                                        Aenean vel justo sit amet velit dapibus ultrices nec at purus. In non finibus nisi. Vestibulum porta lectus dolor, a ullamcorper lorem feugiat varius.
                                                        <br><br>

                                                        <a href="#" class="ui-btn ui-btn-xs-fluid ui-theme-sub ui-fill-dark-100 ui-circle ui-ease-btn">
                                                            Continue
                                                            <svg class="ui-icon ui-m-5-l"><use href="../dist/icons.svg#arrow-right"/></svg>
                                                        </a>
                                                    </p>
                                                </div>

                                            </div>
                                        </div>

                                </div>

                            </div>
                            <div class="ui-carousel-nav ui-form-lg ui-m-10-b ui-ease-1st-btn">
                                <button class="ui-carousel-prev ui-btn ui-btn-ghost ui-btn-square ui-circle">
                                    <svg class="ui-icon ui-m-3-r"><use href="../dist/icons.svg#angle-left"/></svg>
                                </button>
                                <span class="ui-carousel-dots ui-color-black-25"></span>
                                <button class="ui-carousel-next ui-btn ui-btn-ghost ui-btn-square ui-circle">
                                    <svg class="ui-icon ui-m-3-l"><use href="../dist/icons.svg#angle-right"/></svg>
                                </button>
                            </div>
                        </div>

                    </div>
                </div>

                <h5 class="ui-h5 ui-font-bold">Image Carousel</h5>
                <div class="ui-p-30-b">
                    <div class="ui-carousel" data-ui-col="8" data-ui-col-md="4" data-ui-col-sm="3" data-ui-col-xs="1" data-ui-slide="2000">
                        <div class="ui-carousel-slider ui-ease-2nd-layout ui-ease-layout ui-ease-slow ui-ease-in-out">
                            <div class="ui-carousel-content">
                                <img class="ui-photo" src="../public/img/image_10.jpg" alt="">
                            </div>
                            <div class="ui-carousel-content">
                                <img class="ui-photo" src="../public/img/image_02.jpg" alt="">
                            </div>
                            <div class="ui-carousel-content">
                                <img class="ui-photo" src="../public/img/image_08.jpg" alt="">
                            </div>
                            <div class="ui-carousel-content">
                                <img class="ui-photo" src="../public/img/image_04.jpg" alt="">
                            </div>
                            <div class="ui-carousel-content">
                                <img class="ui-photo" src="../public/img/image_05.jpg" alt="">
                            </div>
                            <div class="ui-carousel-content">
                                <img class="ui-photo" src="../public/img/image_06.jpg" alt="">
                            </div>
                            <div class="ui-carousel-content">
                                <img class="ui-photo" src="../public/img/image_07.jpg" alt="">
                            </div>
                            <div class="ui-carousel-content">
                                <img class="ui-photo" src="../public/img/image_08.jpg" alt="">
                            </div>
                            <div class="ui-carousel-content">
                                <img class="ui-photo" src="../public/img/image_09.jpg" alt="">
                            </div>
                            <div class="ui-carousel-content">
                                <img class="ui-photo" src="../public/img/image_10.jpg" alt="">
                            </div>
                            <div class="ui-carousel-content">
                                <img class="ui-photo" src="../public/img/image_01.jpg" alt="">
                            </div>
                            <div class="ui-carousel-content">
                                <img class="ui-photo" src="../public/img/image_02.jpg" alt="">
                            </div>
                            <div class="ui-carousel-content">
                                <img class="ui-photo" src="../public/img/image_03.jpg" alt="">
                            </div>
                            <div class="ui-carousel-content">
                                <img class="ui-photo" src="../public/img/image_04.jpg" alt="">
                            </div>
                            <div class="ui-carousel-content">
                                <img class="ui-photo" src="../public/img/image_05.jpg" alt="">
                            </div>
                            <div class="ui-carousel-content">
                                <img class="ui-photo" src="../public/img/image_06.jpg" alt="">
                            </div>
                        </div>
                        <div class="ui-carousel-nav ui-ease-1st-btn">
                            <button class="ui-carousel-prev ui-btn ui-btn-ghost ui-btn-square ui-circle">
                                <svg class="ui-icon ui-m-3-r"><use href="../dist/icons.svg#angle-left"/></svg>
                            </button>
                            <span class="ui-carousel-dots ui-color-black-25"></span>
                            <button class="ui-carousel-next ui-btn ui-btn-ghost ui-btn-square ui-circle">
                                <svg class="ui-icon ui-m-3-l"><use href="../dist/icons.svg#angle-right"/></svg>
                            </button>
                        </div>
                    </div>

                    <span class="ui-sp-30"></span>

                    <h5 class="ui-h5 ui-font-bold">Half Sized Image Carousel</h5>
                    <div class="ui-carousel ui-carousel-half" data-ui-col="4" data-ui-col-md="3" data-ui-col-sm="2" data-ui-col-xs="2">
                        <div class="ui-carousel-slider ui-ease-layout ui-ease-slow ui-ease-in-out">
                            <div class="ui-carousel-content">
                                <img class="ui-photo" src="../public/img/image_09.jpg" alt="">
                            </div>
                            <div class="ui-carousel-content">
                                <img class="ui-photo" src="../public/img/image_07.jpg" alt="">
                            </div>
                            <div class="ui-carousel-content">
                                <img class="ui-photo" src="../public/img/image_05.jpg" alt="">
                            </div>
                            <div class="ui-carousel-content">
                                <img class="ui-photo" src="../public/img/image_04.jpg" alt="">
                            </div>
                            <div class="ui-carousel-content">
                                <img class="ui-photo" src="../public/img/image_01.jpg" alt="">
                            </div>
                            <div class="ui-carousel-content">
                                <img class="ui-photo" src="../public/img/image_06.jpg" alt="">
                            </div>
                        </div>
                        <div class="ui-carousel-nav ui-m-10-t ui-ease-1st-btn">
                            <button class="ui-carousel-prev ui-btn ui-btn-ghost ui-btn-square ui-circle">
                                <svg class="ui-icon ui-m-3-r"><use href="../dist/icons.svg#angle-left"/></svg>
                            </button>
                            <span class="ui-carousel-dots ui-color-black-25"></span>
                            <button class="ui-carousel-next ui-btn ui-btn-ghost ui-btn-square ui-circle">
                                <svg class="ui-icon ui-m-3-l"><use href="../dist/icons.svg#angle-right"/></svg>
                            </button>
                        </div>
                    </div>
                </div>

            </div>
        </div>
    </div>
</main>
